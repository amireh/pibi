namespace "/accounts/:account_id" do
  condition do
  ...
  [ 'deposits', 'withdrawals', 'recurrings' ].each do |tx_type|
    ...
    namespace "/#{tx_type}" do

      before do
        puts "Type: #{tx_type}"
        current_page(tx_type)
      end
      ...
      # i know i no
      # we HACXs:
      view_prefix = case tx_type
      when 'recurrings'; '/transactions/recurrings'
      else; '/transactions'
      end

      get "/new" do
        @tx = @account.send(tx_type).new
        erb :"#{view_prefix}/new" # ^ hack in place
      end

      get "/:tid/edit" do |_, tid|
        unless @tx = @account.send(tx_type).get(tid)
      ...
      if tx_type == 'recurrings'
        ...
        get '/:tid/toggle_activity' do |_, tid|
...
end # namespace['/accounts/:account_id']
